...
var i = 0;
var count = 0;
var getStockTimer = setInterval(function(){
    var stockInfo = [];
    var index = i;
    if(index == idList.length-1) clearInterval(getStockTimer);
    request(url + idList[i++] + ".html", function(err, res, body){
        ...
        if(!err && res.statusCode == 200){
            ...
            if(isNaN(AHPY)){
                errId.push(id);
                console.log(id + ",第" + (index + 1) + "条数据有误！请手动获取数据");
            }else{
                console.log(id + ",第" + (index + 1) + "条数据插入成功");
            }
            count += 1;
